...
public class SravnjenjePoPopisuController implements ControlledScreen{
	...
	public void initialize(){
		...
		table.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<SravnjenjePoPopisu>() {
			@Override
			public void changed(ObservableValue<? extends SravnjenjePoPopisu> observable, SravnjenjePoPopisu oldValue,
					SravnjenjePoPopisu newValue) {
				if (newValue == null){
		...
		tcBroj.setCellValueFactory(new PropertyValueFactory<SravnjenjePoPopisu, Integer>("Number"));
		tcDate.setCellValueFactory(new PropertyValueFactory<SravnjenjePoPopisu, Date>("Date"));
		tcDate.setCellFactory((TableColumn<SravnjenjePoPopisu, Date> column) -> {
			   return new TableCell<SravnjenjePoPopisu, Date>() {
				      @Override
		...
		tcNapomena.setCellValueFactory(new PropertyValueFactory<SravnjenjePoPopisu, String>("Note"));
	}
	...
	@FXML private TableView<SravnjenjePoPopisu> table;
	@FXML private TableColumn<SravnjenjePoPopisu, Integer> tcBroj;
	@FXML private TableColumn<SravnjenjePoPopisu, Date> tcDate;
	@FXML private TableColumn<SravnjenjePoPopisu, String> tcNapomena;
	...
	@FXML public void actionNew(ActionEvent event) {
		try{
			...
			hm.put("sravnjenjePoPopisu", new SravnjenjePoPopisu(-1, -1, new Date(), 
													  cbDepartment.getSelectionModel().getSelectedItem().getDepartmentID(),
													  ""));
			
			int result = controller.showModalDialog("Sravnjenje po popisu - Detalji", 
									   ScreensController.SCREEN_BACKOFFICE_DOC_SRAVNJENJE_PO_POPISU_DETAILES, hm, 
	...
	@FXML public void actionEdit(ActionEvent event) {
		if (table.getSelectionModel().getSelectedItem() != null){
			HashMap<String, Object> hm = new HashMap<String, Object>();
			hm.put("controller", controller);
			hm.put("sravnjenjePoPopisu", table.getSelectionModel().getSelectedItem());
	...
	private void loadDocumentList(){
		...
		try{
			if (cbDepartment.getSelectionModel().getSelectedItem().getDepartmentID() > 0){
				table.setItems(SravnjenjePoPopisu.getlist(cbDepartment.getSelectionModel().getSelectedItem(), 
													 controller.getSettings().getConnection()));
			}
